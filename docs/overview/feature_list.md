...
| F01-03 | | 選択を壊す報告の抑止 | xterm.js はユーザー入力のたびに選択を破棄し、マウス報告もユーザー入力として扱われるため、報告 1 つで選択が消える。`triggerMouseEvent` を包み、Shift 押下中はすべて、選択がある間は移動と離しの報告を抑止する（押下とホイールは通す） | `frontend/js/terminal_manager.js` |
